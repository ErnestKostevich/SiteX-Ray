...
## 3. NOWPayments (crypto checkout)

NOWPayments is a crypto payment processor that lets customers pay in 100+ cryptos (BTC, ETH, USDT, USDC, etc.). You receive crypto in your own wallet. Fee: 0.5% per sale.

**KYC note:** receiving crypto to your own wallet is non-custodial and doesn't require KYC. KYC only kicks in if you opt into NOWPayments' fiat-conversion service. For pure-crypto operation you stay anonymous.

### Setup (~10 minutes)

You already have an account at https://account.nowpayments.io/dashboard, so we just need to grab two credentials and wire one webhook.

1. **Log in** → https://account.nowpayments.io/dashboard
...
3. **Store → IPN settings**:
   ...
   - Save.
4. **Store → Coins** — make sure your payout coin is enabled. Recommended: **USDT (TRC20)** — Tron network has ~$1 fees and instant settlement. Or **USDC (SOL)** for solana.
5. **Wallet → Add wallet** → enter your personal USDT-TRC20 (or USDC-SOL) address where payouts will land.
6. (Optional) Switch on **Sandbox mode** in account settings to test with free testnet coins before going live.

### Add the env vars to Cloudflare Pages

Cloudflare Pages → Settings → Environment variables → Production → add (both encrypted):
- `NOWPAYMENTS_API_KEY` — from step 2
- `NOWPAYMENTS_IPN_SECRET` — from step 3

Then **Retry deployment** so functions pick up the env vars. The landing's "Get full audit — $39" button is already wired to call `/api/create-invoice` which creates a NOWPayments invoice and redirects the customer to NOWPayments' hosted checkout. After payment, NOWPayments calls `/api/nowpayments-webhook` and your full audit is delivered by email.

No edits to `landing/index.html` required — checkout is fully server-side. Customer URL + email are encoded into the NOWPayments `order_id` field (URL-safe base64) and decoded by the webhook — no database needed.
...
3. You'll get redirected to a NOWPayments checkout page. Pick any crypto.
4. Use testnet coins (NOWPayments sandbox provides faucet info per coin) to pay
5. Once confirmed, you should get redirected to `/success.html` and the FULL audit should arrive by email within 2–3 minutes
6. When everything works → switch off Sandbox to accept real payments
...
| NOWPayments webhook fires but no email goes out | Audit pipeline crashed; check Cloudflare → Pages → Functions logs | Likely a malformed `order_id` or missing AI backend |
